...
totalRecords.forEach(function(item){
  ...
  if (item.hasOwnProperty('position')) {
    ...
    var geoJsonFeature = { "type":"Feature", "properties":{ "ingress": ingress.substring(0, 50), "date":item.created, "author":item.contributor.display_name }, "geometry": { "type":"Point", "coordinates": [ parseFloat(item.position.longitude.toFixed(3)), parseFloat(item.position.latitude.toFixed(3)) ] } };
